...
export const SaveraLogo: React.FC<SaveraLogoProps> = ({ className = '', size = 'md' }) => {
  const dimensions = {
    sm: { box: 'w-10 h-10', svg: 38 },
    md: { box: 'w-14 h-14', svg: 52 },
    lg: { box: 'w-20 h-20', svg: 76 },
  }[size];

  return (
    <div className={`flex items-center justify-center ${dimensions.box} ${className}`}>
      {/* Exact corporate spiral 'S' logo in bright cyan/blue on white background */}
      <div className="w-full h-full bg-white rounded-lg shadow-md flex items-center justify-center p-1 border border-slate-200">
        <svg
          viewBox="0 0 100 100"
        ...
        >
          <defs>
            ...
            <filter id="saveraShadow" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="2" stdDeviation="2" floodColor="#0369a1" floodOpacity="0.25" />
            </filter>
          </defs>

          {/* Dynamic swirling corporate "S" emblem from the original Savera PDF */}
          <path
